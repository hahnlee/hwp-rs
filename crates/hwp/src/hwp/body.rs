...
use super::{section::Section, version::Version};
...
impl Body {
    pub fn from_cfb<T: Read + Seek>(cfb: &mut CompoundFile<T>, version: &Version) -> Body {
        ...
        for i in 0..size {
            ...
            let section = Section::from_stream(&mut stream, version);
    ...
    pub fn from_distributed<T: Read + Seek>(cfb: &mut CompoundFile<T>, version: &Version) -> Body {
        ...
        for i in 0..size {
            ...
            let section = Section::from_distributed(&mut stream, version);
            sections.push(section);
        }
